...
inline std::string tokToString(const Token& tok) {
  switch (tok) {
    ...
    case Token::tok_bracket_close : return "tok_bracket_close";
    ...
    case Token::tok_equals : return "tok_equals";
    case Token::tok_comma : return "tok_comma";
    case Token::tok_print : return "tok_print";
    default: return "";
  }
}
...
inline bool areToksEqual (const std::vector<TokType>& v1, const std::vector<TokType>& v2) {
  // check size
  if (v1.size() != v2.size()) {
    return false;
  }
  ...
  for (size_t i = 0; i < v1.size(); i++) {
    ...
    if (std::holds_alternative<TokWithLieral>(v1[i])) {
      if ((std::get<TokWithLieral>(v1[i]).tok != std::get<TokWithLieral>(v2[i]).tok)
            || (std::get<TokWithLieral>(v1[i]).literal != std::get<TokWithLieral>(v2[i]).literal)) {
        return false;
      }
    }
  }
